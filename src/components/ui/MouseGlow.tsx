...
export default function MouseGlow({ hide = false }: MouseGlowProps) {
  const cursorRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!cursorRef.current || hide) return;

    class ArrowPointer {
      root: HTMLElement;
      cursor: HTMLElement;
      position: { distanceX: number; distanceY: number; distance: number; pointerX: number; pointerY: number; };
      ...
      move(event: MouseEvent) {
        ...
        this.position.pointerX = event.pageX - window.scrollX; // Use pageX and adjust against scroll for fixed positioning
        this.position.pointerY = event.pageY - window.scrollY;
        
        // Wait, the original code used pageX + getBoundingClientRect().x ...
        // Since we are using "fixed" position, it's safer to just use clientX to avoid scroll offset bugs.
    ...
    const arrowCursor = new ArrowPointer(cursorRef.current);
    
    const onMouseMove = (event: MouseEvent) => {
      arrowCursor.move(event);
    };
