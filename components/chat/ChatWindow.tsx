...
export default function ChatWindow({ conversationId }: { conversationId?: string }) {
  ...
  const handleBranch = async (messageId: string) => {
      ...
      // Visually keep history up to the branched message instantly
      setMessages(messages.slice(0, index + 1));
      
      // Hit the database to clone the Conversation
      if (branchConversation) {
          const newConvoId = await branchConversation(messageId);
          if (newConvoId) {
             window.history.pushState({}, '', `/chat/${newConvoId}`);
          } else {
             // Fallback local branch if backend fails
  ...
  const handleSend = async (content: string, scheduledFor?: Date) => {
      if (scheduledFor) {
  ...
  return (
    <div className="relative flex flex-col h-full w-full overflow-hidden bg-[#eef2f9]">
      {/* Stronger pastel orbs for better contrast */}
      ...
      <div
        ref={scrollRef}
        className="relative z-10 flex-1 overflow-y-auto w-full custom-scrollbar pt-6 pb-2"
      >
        <div className="max-w-3xl mx-auto px-6 w-full pb-8">
            {messages.length === 0 && (
      ...
      <InputArea onSend={handleSend} isLoading={isLoading} selectedModel={selectedModel} onModelChange={setSelectedModel} />
    </div>
  );
}
