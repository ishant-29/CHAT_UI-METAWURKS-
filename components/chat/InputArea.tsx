...
import { Send, Paperclip, X, FileText, Image as ImageIcon, Mic, Video } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import ModelSelector from "./ModelSelector";
import { LLMModel } from "@/lib/constants/models";

interface Props {
  onSend: (content: string, scheduledFor?: Date) => void;
  isLoading: boolean;
...
export default function InputArea({ onSend, isLoading, selectedModel, onModelChange }: Props) {
  ...
  const handleSend = () => {
    ...
    // In a real app we'd process attachments here too
    onSend(input.trim() || "[Sent with attachment]");
  ...
  const triggerUpload = (filter: string) => {
      ...
      // Let React state update before clicking the native input
      setTimeout(() => {
